...
def ICON_laplacian_diamond(
    ...
    for _ in forward:
        ...
        for _ in neighbors[Edge > Cell > Vertex]:
            vn_vert = (
                u_vert * primal_normal_x
                + v_vert * primal_normal_y
            )
...
@stencil
def h_offsets(a: Field[Edge > Cell > Edge], b: Field[Edge], c: Field[Edge > Cell > Edge]):
    for _ in forward:
        for _ in neighbors[Edge > Cell > Edge]:
            a = b + c   # no offsets, defaults to True
            a = b[Edge > Cell > Edge] + c[Edge > Cell >
                                          Edge]  # verbose version of the above
            a = b[Edge] + c[Edge > Cell >
                            Edge]  # center access for dense field
...
@stencil
def hv_offsets(a: Field[Edge > Cell > Edge], b: Field[Edge], c: Field[Edge > Cell > Edge]):
    for k in forward:
        for _ in neighbors[Edge > Cell > Edge]:
            a = b[k] + c   
            a = b[Edge > Cell > Edge, k+1] + c[Edge > Cell >
                                          Edge, k]
            a = b[Edge, k] +  b[Edge, k-1]+c[Edge > Cell >
                            Edge]  